(ns ovenfurnacemfg.operation
  "OvenFurnaceOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (OvenFurnaceAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Oven &
  Furnace Plant Operations Governor (:govern) and the rollout phase
  ...
  (:require [langgraph.graph :as g]
            ...
            [ovenfurnacemfg.advisor :as advisor]
...
(defn build
  "Compiles an OvenFurnaceOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `ovenfurnacemfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; OvenFurnaceAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Oven & Furnace Plant Operations Governor -- independent censor.
      (g/add-node :govern
