(ns ovenfurnacemfg.advisor
  "OvenFurnaceAdvisor -- the *contained intelligence node* for the
  industrial oven/furnace/furnace-burner plant-operations coordination
  actor.

  It normalizes production-batch patches (product-type/thermal-test-
  degc/quantity/defect-rate), drafts a fabrication/assembly/thermal-
  ...
  industrial-oven/furnace/furnace-burner shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real fabrication/
  assembly-line-equipment actuation, freight dispatch, or safety
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `ovenfurnacemfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `ovenfurnacemfg.governor`)
  ...
  `ovenfurnacemfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [ovenfurnacemfg.registry :as registry]
...
(defn- log-production-batch
  ...
  thermal-test-degc, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a fabrication/assembly/thermal-test-bench-equipment
  ...
  registered?) in its rationale, but `ovenfurnacemfg.governor` NEVER
...
(defn- flag-safety-concern
  "Draft an equipment-safety/burner-combustion-safety concern. ALWAYS
  ...
  `ovenfurnacemfg.phase`: no phase ever adds this op to a phase's
  `:auto` set; `ovenfurnacemfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound industrial-oven/furnace/furnace-burner shipment
  ...
  one, and `ovenfurnacemfg.governor` NEVER trusts it: it independently
...
(def ^:private system-prompt
  (str "あなたは工業用オーブン・炉・バーナー(工業用オーブン・熱処理炉・溶解炉・"
       "バーナー等の工業用加熱機器)製造工場プラント運用コーディネーターの助言者です。"
       ...
       "製缶・組立・熱試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "UL 795/UL 726・CSA・CE(EU Gas Appliances Regulation 2016/426等)の"
       "燃焼機器安全認証を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `ovenfurnacemfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :ovenfurnacemfg-advisor-proposal
   :op         (:op request)
