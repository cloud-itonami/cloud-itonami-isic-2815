(ns ovenfurnacemfg.governor
  "Oven & Furnace Plant Operations Governor -- the independent
  compliance layer that earns the OvenFurnaceAdvisor the right to
  ...
  proposal secretly tries to self-issue a UL/CSA/CE combustion-
  equipment safety-certification mark (an authority this actor never
  ...
  act stops being a coordination proposal and becomes direct
  fabrication/assembly-line-equipment control, so this MUST be a
  separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:oven-furnace-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       UL/CSA/CE combustion-equipment
                                       safety-certification mark (e.g.
                                       UL 795 commercial-industrial
                                       gas-fired equipment / UL 726
                                       oil-fired equipment listing, CSA
                                       certification, CE marking under
                                       the EU Gas Appliances Regulation
                                       2016/426) -- an authority
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`ovenfurnacemfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid thermal-test-degc   -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:thermal-test-degc` that is not
                                       a physically plausible
                                       thermal-test reading
                                       (`ovenfurnacemfg.registry/
                                       thermal-test-degc-valid?`), the
   ...
   12. Invalid defect-rate         -- for `:log-production-batch`, if
                                       ...
                                       reading (`ovenfurnacemfg.registry/
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [ovenfurnacemfg.registry :as registry]
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は製缶・組立設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate fabrication/assembly-line equipment -- this actor
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "製缶・組立・熱試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a UL/CSA/CE combustion-equipment safety-certification
  mark (e.g. UL 795 commercial-industrial gas-fired equipment / UL 726
  oil-fired equipment listing, CSA certification, CE marking under the
  EU Gas Appliances Regulation 2016/426) -- an authority exclusively
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "UL 795/UL 726・CSA・CE(EU Gas Appliances Regulation 2016/426等)の燃焼機器安全認証の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-thermal-test-degc-violations
  "For `:log-production-batch`, if the patch declares a
  `:thermal-test-degc` that is not a physically plausible thermal-test
  reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [degc (:thermal-test-degc (:value proposal))]
      (when (and (some? degc) (not (registry/thermal-test-degc-valid? degc)))
        [{:rule :invalid-thermal-test-degc
          :detail (str degc "degC は物理的に妥当な熱試験値の範囲外")}]))))
...
(defn check
  "Censors an OvenFurnaceAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-thermal-test-degc-violations request proposal)
