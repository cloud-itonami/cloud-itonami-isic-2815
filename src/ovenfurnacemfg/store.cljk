(ns ovenfurnacemfg.store
  "SSoT for the industrial oven/furnace/furnace-burner (industrial
  ovens, heat-treatment furnaces, melting furnaces, furnace burners)
  plant-operations coordination actor, behind a `Store` protocol so the
  ...
  is self-contained (no external oven/furnace-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/thermal-test-degc/quantity/
    ...
    - `equipment`         -- a fabrication-line/assembly-line/thermal-
    ...
    - `maintenance`       -- a scheduled fabrication/assembly/thermal-
                             test-bench-equipment maintenance-window
                             DRAFT against a piece of equipment
                             (`ovenfurnacemfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound oven/furnace/furnace-
                             burner shipment DRAFT
                             (`ovenfurnacemfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (ovenfurnacemfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (ovenfurnacemfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :industrial-oven
                :model "IO-A600" :thermal-test-degc 850.0
                :quantity-units 400.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :heat-treatment-furnace
                :model "HTF-T12" :thermal-test-degc 1150.0
                :quantity-units 120.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 115.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :furnace-burner
                :model "FB-R25" :thermal-test-degc 600.0
...
(defn- sample-equipment []
  {"fab-001" {:id "fab-001" :kind :fabrication-line
   ...
   "bench-002" {:id "bench-002" :kind :thermal-test-bench
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `ovenfurnacemfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `ovenfurnacemfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  UNVERIFIED/unregistered thermal-test-bench unit (blocks any
