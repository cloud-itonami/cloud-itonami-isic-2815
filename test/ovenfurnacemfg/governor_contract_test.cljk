(ns ovenfurnacemfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control fabrication/assembly-line equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue a UL/CSA/CE combustion-equipment
  safety-certification mark') implemented faithfully. The single
  invariant under test:

    OvenFurnaceAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Oven & Furnace Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :industrial-oven}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :industrial-oven (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the UL/CSA/CE combustion-equipment safety-certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-thermal-test-degc-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:thermal-test-degc 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-thermal-test-degc} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:thermal-test-degc (store/batch db "batch-001"))) "fabricated thermal-test-degc never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "fab-001" :severity :moderate
                                            :description "burner combustion anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
